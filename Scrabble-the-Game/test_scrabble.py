...
def test_compChooseWord(wordList):
    ...
    temp = compChooseWord({'a': 1, 'p': 2, 's': 1, 'e': 1, 'l': 1}, wordList, 6)
    if temp not in ['appels', 'apples']:
        print "FAILURE: test_compChooseWord()"
        print "\tExpected 'appels' or 'apples', but got: '" + temp + "'"
        
        failure = True
    
    temp = compChooseWord({'a': 2, 'c': 1, 'b': 1, 't': 1}, wordList, 5)
    if temp != 'acta':
        print "FAILURE: test_compChooseWord()"
        print "\tExpected 'acta', but got: '" + temp + "'"
        
        failure = True 
        
    temp = compChooseWord({'a': 2, 'e': 2, 'i': 2, 'm': 2, 'n': 2, 't': 2}, wordList, 12)
    if temp != 'immanent':
        print "FAILURE: test_compChooseWord()"
        print "\tExpected 'immanent', but got: '" + temp + "'"
        
        failure = True 
        
    temp = compChooseWord({'x': 2, 'z': 2, 'q': 2, 'n': 2, 't': 2}, wordList, 12)
    if temp != None:
        print "FAILURE: test_compChooseWord()"
        print "\tExpected None, but got: '" + temp + "'"
        
        failure = True 
        
    if not failure:
        print "SUCCESS: compChooseWord()"
